...
`` /sites/changeDate: ``
С помощью этого запроса можно изменить дату, передав её в этот запрос, для неё подготовятся данные и начнется стрим данных уже для новой даты.
* Входные данные: дата


`` /sites/deactivate: ``
Этот запрос удаляет все данные и сервисы и прекращается стриминг данных.
...
3.  Streams: передается список станций и если эти станции есть и их имя введено правильно, то будет выдана тема для прослушивания этих станций
4. ChangeDate: можно изменить дату, передав  её в этот запрос, для неё подготовятся данные и начнется стрим данных уже для новой даты
5. Deactivate: удаляет все данные и сервисы и прикрашается стриминг данных
