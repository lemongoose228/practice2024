...
4. Создаётся подписка на темы
5. Стримится дата
6. ChangeDate: можно изменить дату, передав  её в этот запрос, для неё подготовятся данные и начнется стрим данных уже для новой даты
7. Deactivate: удаляет все данные и сервисы и прикрашается стриминг данных
